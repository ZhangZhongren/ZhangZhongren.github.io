...
webpack 仅仅是一个打包工具，他能帮我做的事情也就是文件打包处理。

## 安装 webpack

> npm i webpack -g  全局安装

> npm i webpack -D  项目下安装

::: tip
项目下运行 webpack  命令  npx webpack
:::

webpack 可以事项无配置打包，默认去找src/index.js


## webpack.config.js

运行 webpack 自动回去寻找项目下的 webpack.config.js
...
```
1. entry 入口文件 支持单入口和多入口， 多入口传入一个数组
2. output 打包之后的文件输出位置
3. module 打包的配置 可以配置 rules等
4. plugins 插件
5. devServer 如果需要本地热更新 安装 webpack-dev-serve 之后 配置
6. mode 打包模式
...
+ css-loader 处理 css 识别 .css文件 并且能识别 vue 组建 style 标签里面的样式
+ style-loader 将编译好的css插入到html中
+ vue-loader 识别 .vue 文件
+ file-loader 处理文件
+ uil-loader file-loader 的升级版 使用之前 需要安装 file-loader 可以配置 一个范围 超过范围的 超过限制大小的才会打包成静态文件

等等 慢慢补充

## webpack 插件

+ clean-webpack-plugin 每次打包之前都晴空 打包的出口目录
+ html-webpack-plugin 出口html文件 [github](https://github.com/jantimon/html-webpack-plugin)
+ vue-loader/lib/plugin 使用vue-loader 必须要依赖的一个插件
+ webpack.DefinePlugin 写入全局变量 项目中使用该变量的地方 打包过程中会被一一替换掉

## 第一个webpack 工程化项目

逐渐完善中
[mywebpack](https://github.com/ZhangZhongren/webpack-for-vue)
...
webpack3.x的版本与webpack-dev-server3.x 的版本不兼容 否则报错 
